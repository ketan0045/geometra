...
export default createStore({
    ...
    initValueInheritance(projectId) {
        ...
        const sessionValues = JSON.parse(localStorage.getItem(`${this.activeProjectId}_inheritance`));
    ...
    changeAnnotationInheritance({ type, styles, properties }) {
        ...
        localStorage.setItem(
            `${this.activeProjectId}_inheritance`,
            JSON.stringify(Object.assign({}, JSON.parse(localStorage.getItem(`${this.activeProjectId}_inheritance`)), this.typeInheritanceMap.toJS()))
        );
    },
    ...
    initializeEventHandlers() {
        if (this.WebViewer) {
            ...
            annotationManager.addEventListener("annotationSelected", (annotationList, action) => {
                ...
                if (this.currentToolMode === "Pan") return annotationManager.setReadOnly(false);
                switch (action) {
                    case "selected":
                        const newlySelected = [];
    ...
    changeFormula(annotationsImmutableList, formula, formulaVariables, projectId) {
        if (annotationsImmutableList) {
            ...
            const parser = new DOMParser();
            const oSerializer = new XMLSerializer();
            const standardAnnots = annotationsImmutableList.filter((annot) => annot.type !== "3DModel");

            standardAnnots.forEach((storeAnnotation) => {
                const xfdfElements = parser.parseFromString(storeAnnotation.xfdf, "text/xml");
                let updated = false;
                const annotElement = xfdfElements.querySelector("annots").firstElementChild;
                switch (this.getTypeConversion(storeAnnotation.type)) {
                    case "Polygon":
                    case "Reduction":
                    case "Ellipse":
                    case "Free hand":
                ...
                if (updated) {
                    const updatedXFDF = oSerializer.serializeToString(xfdfElements);
                    annotationsToUpdate.push(storeAnnotation);
                }
            });
    ...
    initPdfAnnotationAsDeselected(pdfAnnotation) {
        ...
        if (pdfAnnotation) {
            ...
            switch (pdfAnnotation.annotationType) {
                case "Point":
                case "Polygon":
                case "Ellipse":
                case "annotation.freeHand":
    ...
    initPdfAnnotationAsSelected(pdfAnnotation) {
        ...
        if (pdfAnnotation) {
            pdfAnnotation.Opacity = 1.0;
            pdfAnnotation.StrokeColor = { A: 1.0 };
            switch (pdfAnnotation.annotationType) {
                case "Point":
                case "Polygon":
                case "Ellipse":
    ...
    async loadAnnotations() {
        try {
            ...
            const scales = ObjectsStore.getAllScales(activeEstimateId, this.ActiveFileId);

            _.forEach([...annotations, ...reductions, ...scales], (annot) => {
                if (annot.type !== ANNOT_TYPES.GROUP && this.ActiveFileId == annot.geoFile.id) {
                    if (annot.type === ANNOT_TYPES.Y_SCALE || annot.type === ANNOT_TYPES.X_SCALE) {
                        annotComHandler.addModifyCommand(annot.xdf);
                    } else {
    ...
    async setToolMode(toolMode, isHotkeyAction = false) {
        ...
        if (isEstimateLocked && !isViewTool) {
            ...
            await this.WebViewer.setToolMode("AnnotationEdit");

            return;
        }
        if (toolMode === "AnnotationCreateReduction") this.setIsReductionCreated(true);
        this.currentToolMode = toolMode;
        await this.WebViewer.setToolMode(toolMode);
        if (toolMode === "AnnotationEdit" && this.newAddedAnnot) {
            await ObjectsStore.selectAnnotation(this.newAddedAnnot);
            this.newAddedAnnot = undefined;
        }
    },
    ...
    selectAll(isAltPressed) {
        ...
        const objectsToSelect = _.filter(annotsToSelect, (annot) => !_.some(mainFoldersToSelect, (folder) => folder.id === annot.id));
    ...
    onRequestAnnotationUpdateArray(annotationList, key, value) {
        ...
        annotationList.forEach((annotation) => {
            const geoFileId = annotation.geoFile.id;
            const geoEstimateId = annotation.geoEstimate.id;
            const annotationType = annotation.type;
            const geoAnnotId = annotation.id;
            if (annotationType !== "x-scale" && annotationType !== "y-scale") {
                let storeAnnotation = undefined;
                if (annotationType === "CenterValue" || annotationType === "PeripheralValue") {
                    storeAnnotation = this.getDisplayValueAnnotationByAnnotationId(geoAnnotId);
                } else if (annotationType === "Reduction") {
                    storeAnnotation = ObjectsStore.getReductionByPDFTronAnnot({ geoEstimateId, geoFileId, geoAnnotId, geoParentId: annotation.parentId });
                } else {
                    storeAnnotation = ObjectsStore.getAnnotationByPDFTronAnnot({ geoEstimateId, geoFileId, geoAnnotId });
                }
                if (storeAnnotation) {
                    let annotationUpdated = false;
                    if (key && value !== annotation[key]) {
                    ...
                    if (!key || (key && annotationUpdated)) {
                        const geoAnnotation = { ...storeAnnotation, ...annotation };
                        ...
                        if (geoAnnotation.xfdf && geoAnnotation.type !== "3DModel") {
                            ...
                            this.changeAnnotationInheritance({ type: annotationType, styles, properties });
                    ...
                    case "Free text":
                        {
                            ...
                            currentTextFont = this.freeTextValueInheritance.getTextFont();
                            ...
                            annotationElement.setAttribute("textFont", currentTextFont);
    ...
    async onAnnotationsUpdateHandler(response) {
        const { parameter, value, updateAction, path } = response;
        try {
            const annotComHandler = new AnnotCommandHandler();
            ...
            _.forEach(updatedAnnots, (geoAnnotation) => {
                const annotationType = geoAnnotation.type;
                ObjectsStore.exchangeSelectedIfSelected(geoAnnotation);
                if (
                    (annotationType !== ANNOT_TYPES.GROUP || annotationType !== ANNOT_TYPES.IFC_MODEL) &&
                    geoAnnotation.geoFile?.id === this.getActiveFileId()
                ) {
                    annotComHandler.addModifyCommand(geoAnnotation.xfdf);
                    if (geoAnnotation.xfdf && annotationType !== "3DModel") {
                        const parser = new DOMParser();
                        const oSerializer = new XMLSerializer();
                        const xfdfElements = parser.parseFromString(geoAnnotation.xfdf, "text/xml");
                        const annotation = xfdfElements.querySelector("annots").children[0];
                        ...
                        let labels = { ...geoAnnotation.labels };
                        if (parameter === "labels") {
                            try {
                                labels = this.updateLabels(labels, path, updateAction, value);
                            } catch (e) {
                                console.log("Error with labels inheritance: ", e);
                                labels = null;
                            }
                        } else if (parameter && value !== geoAnnotation[parameter]) {
                            geoAnnotation[parameter] = value;
                            const valueInheritance = this.typeInheritanceMap.get(this.getTypeConversion(annotationType));
                            if (valueInheritance) {
                                switch (parameter) {
                                    case "name":
    ...
    getLabelsByAnnotType(type) {
        switch (type) {
            ...
            case ANNOT_TYPES.POLYLINE:
                return {
                    sideLabels: [LABELS.NR_TAG, LABELS.NAME, LABELS.NET_LENGTH, LABELS.NET_WALL, LABELS.LENGTHS, LABELS.WALLS],
                };
