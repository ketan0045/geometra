...
import replacements from "../utils/MathjsConfig";
...
export default createStore({
    ...
    setUnitForRow(row) {
        let currentIndex = Number.MAX_SAFE_INTEGER;
        let currentUnit = undefined;
        if (row && row?.amount) {
            let tempIndex = undefined;
            const lowerCaseExp = row.amount.toLowerCase();
            const mUnitIfcRegex = new RegExp(/(l|sl|v)[0-9]+/);
            const m2UnitIfcRegex = new RegExp(/(s)[0-9]+/);
            if (lowerCaseExp.split(" ").length > 1) {
                const split = lowerCaseExp.split(" ");
                if (split.length > 1) {
                    currentUnit = split[1];
                    return currentUnit;
                }
            }
            tempIndex = lowerCaseExp.indexOf("a");
            if (tempIndex >= 0 && tempIndex < currentIndex) {
                currentIndex = tempIndex;
                currentUnit = "m2";
            }
            tempIndex = lowerCaseExp.indexOf("na");
            if (tempIndex >= 0 && tempIndex < currentIndex) {
                currentIndex = tempIndex;
                currentUnit = "m2";
            }
            tempIndex = lowerCaseExp.indexOf("l");
            if (tempIndex >= 0 && tempIndex < currentIndex) {
                currentIndex = tempIndex;
                currentUnit = "m";
            }
            tempIndex = lowerCaseExp.indexOf("nl");
            if (tempIndex >= 0 && tempIndex < currentIndex) {
                currentIndex = tempIndex;
                currentUnit = "m";
            }
            tempIndex = lowerCaseExp.indexOf("vo");
            if (tempIndex >= 0 && tempIndex < currentIndex) {
                currentIndex = tempIndex;
                currentUnit = "m3";
            }
            tempIndex = lowerCaseExp.indexOf("nvo");
            if (tempIndex >= 0 && tempIndex < currentIndex) {
                currentIndex = tempIndex;
                currentUnit = "m3";
            }
            tempIndex = lowerCaseExp.indexOf("v");
            if (tempIndex >= 0 && tempIndex < currentIndex) {
                currentIndex = tempIndex;
                currentUnit = "m2";
            }
            tempIndex = lowerCaseExp.indexOf("rl");
            if (tempIndex >= 0 && tempIndex < currentIndex) {
                currentIndex = tempIndex;
                currentUnit = "m";
            }
            tempIndex = lowerCaseExp.indexOf("r");
            if (tempIndex >= 0 && tempIndex < currentIndex) {
                currentIndex = tempIndex;
                currentUnit = "m2";
            }
            tempIndex = lowerCaseExp.indexOf("p");
            if (tempIndex >= 0 && tempIndex < currentIndex) {
                currentIndex = tempIndex;
                currentUnit = "st";
            }
            tempIndex = lowerCaseExp.indexOf("h");
            if (tempIndex >= 0 && tempIndex < currentIndex) {
                currentIndex = tempIndex;
                currentUnit = "m";
            }
            tempIndex = lowerCaseExp.indexOf("q");
            if (tempIndex >= 0 && tempIndex < currentIndex) {
                currentIndex = tempIndex;
                currentUnit = "st";
            }
            tempIndex = lowerCaseExp.indexOf("dx");
            if (tempIndex >= 0 && tempIndex < currentIndex) {
                currentIndex = tempIndex;
                currentUnit = "m";
            }
            tempIndex = lowerCaseExp.indexOf("dy");
            if (tempIndex >= 0 && tempIndex < currentIndex) {
                currentIndex = tempIndex;
                currentUnit = "m";
            }
            tempIndex = lowerCaseExp.indexOf("t");
            if (tempIndex >= 0 && tempIndex < currentIndex) {
                currentIndex = tempIndex;
                currentUnit = "st";
            }
            tempIndex = lowerCaseExp.indexOf("svo");
            if (tempIndex >= 0 && tempIndex < currentIndex) {
                currentIndex = tempIndex;
                currentUnit = "m3";
            }
            tempIndex = lowerCaseExp.indexOf("sl");
            if (tempIndex >= 0 && tempIndex < currentIndex) {
                currentIndex = tempIndex;
                currentUnit = "m";
            }
            if (AnnotationStore.getType() === "3DModel" && m2UnitIfcRegex.test(lowerCaseExp)) {
                currentUnit = "m2";
            }
            if (AnnotationStore.getType() === "3DModel" && mUnitIfcRegex.test(lowerCaseExp)) {
                currentUnit = "m";
            }
        }
        return currentUnit;
    ...
    onRequestDeleteRow(annotations) {
        if (annotations) {
            let payload = {
                ids: [],
            };
            annotations.forEach((annot) => {
                if (annot.rows && Object.values(annot.rows).length) {
                    const annotationRowIds = Object.values(annot.rows).map((annotationRow) => {
                        return annotationRow.id;
                    });
                    payload.ids = payload.ids.concat(annotationRowIds);
                }
            });
