...
export default createStore({
    ...
    getNextProjectNumber(rowNum) {
        const calculator = new GeometricCalculation();
        let currentProjectNr = rowNum || this.nextAvailableNr || this.projects?.get(this.projects?.size - 1)?.get("projectNumber");
        if (currentProjectNr) {
            let data = calculator.getNextNumberString(currentProjectNr);
            this.nextAvailableNr = data;
            return data;
        } else {
