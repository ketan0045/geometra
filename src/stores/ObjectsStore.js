...
export default createStore({
    ...
    createObjectsMap(annotations, reductions, folders, scales, estimate) {
        ...
        const scalesMap = this.createScalesIdMap(scales, estimate);
    ...
    createScalesIdMap(scales, estimate) {
        let fileIdMapForScales = this.createEmptyFileIdMap(scales);

        _.forEach(scales, (scale) => {
            const parsedScale = this.parseScale(scale, estimate);
    ...
    onReductionCreate(reductionData) {
        ...
        if (Boolean(this.estimateObjectsHashMap[geoEstimateId].objects.reductionsMap[fileId])) {
            this.estimateObjectsHashMap[geoEstimateId].objects.reductionsMap[fileId][parentId]
                ? Object.assign(this.estimateObjectsHashMap[geoEstimateId].objects.reductionsMap[fileId][parentId], { [id]: annotationData })
                : Object.assign(this.estimateObjectsHashMap[geoEstimateId].objects.reductionsMap[fileId], { [parentId]: { [id]: annotationData } });
        } else {
            Object.assign(this.estimateObjectsHashMap[geoEstimateId].objects.reductionsMap, { [fileId]: { [parentId]: { [id]: annotationData } } });
        }
    ...
    onScaleCreate(scale) {
        ...
        if (Boolean(this.estimateObjectsHashMap[geoEstimateId].objects.scalesMap[fileId])) {
            this.estimateObjectsHashMap[geoEstimateId].objects.scalesMap[fileId][page]
                ? Object.assign(this.estimateObjectsHashMap[geoEstimateId].objects.scalesMap[fileId][page], { [type]: parsedScale })
                : Object.assign(this.estimateObjectsHashMap[geoEstimateId].objects.scalesMap[fileId], { [page]: { [type]: parsedScale } });
        } else {
            Object.assign(this.estimateObjectsHashMap[geoEstimateId].objects.scalesMap, { [fileId]: { [page]: { [type]: parsedScale } } });
        }
    ...
    onEditEstimate(id, parameter, value) {
        this.estimateObjectsHashMap[id].estimateData[parameter] = value;
        if (parameter === "locked") {
            const allAnnotationsToLock = [...this.getAllAnnotations(id), ...this.getAllReductions(id), ...this.getAllScales(id)];
            _.forEach(allAnnotationsToLock, (annot) => {
                let parsedAnnot;
                annot["readOnly"] = value;
                if (annot.type === ANNOT_TYPES.Y_SCALE || annot.type === ANNOT_TYPES.X_SCALE) {
                    annot.xdf = this.parseAnnotationAttribute(annot.xdf, "setInitialScaleValues", annot);
                    parsedAnnot = this.parseScale(annot, this.estimateObjectsHashMap[id].estimateData, { readOnly: value });
                } else {
                    annot.xfdf = this.parseAnnotationAttribute(annot.xfdf, "setInitialValues", annot);
                    parsedAnnot = this.parseAnnotation(annot, this.estimateObjectsHashMap[id].estimateData);
                }
                this.updateAnnotInHashMap(parsedAnnot);
            });
            AnnotationStore.loadAllAnnotations();
        }
    },
        ...
        _.forEach(annotData, (updatedAnnotData) => {
            ...
            switch (type) {
                case ANNOT_TYPES.IFC_MODEL:
                    const ifcAnnot = { ...this.getAnnotationByPDFTronAnnot({ geoEstimateId, geoFileId: geoFile.id, geoAnnotId: id }) };
                ...
                case ANNOT_TYPES.GROUP:
                    ...
                    break;
                case ANNOT_TYPES.REDUCTION:
                    const reduction = { ...this.getReductionByPDFTronAnnot({ geoEstimateId, geoFileId: geoFile.id, geoAnnotId: id, geoParentId: parentId }) };
                    ...
                    break;
                default:
                    const annotation = { ...this.getAnnotationByPDFTronAnnot({ geoEstimateId, geoFileId: geoFile.id, geoAnnotId: id }) };
                    const updatedAnnotation = this.updatePdfAnnotation({ annotation, parameter, value, path, updateAction, xfdf });
    ...
    updatePdfAnnotation({ annotation, parameter, value, path, updateAction, xfdf }) {
        ...
        if (_.includes(objectParameters, parameter)) {
            ...
            let xfdfElements = parser.parseFromString(annotation.xfdf, "text/xml");
            if (annotation.type === ANNOT_TYPES.FREE_TEXT2 || annotation.type === ANNOT_TYPES.FREE_TEXT)
                xfdfElements = parser.parseFromString(xfdf, "text/xml");
            ...
            const parsedRow = this.parseRows(annotation.rows, annotation);
            parsedAnnot.rows = { ...annotation.rows, ...parsedRow };
    ...
    updateScale(payload) {
        ...
        const parsedScale = this.parseScale(activeScale, this.estimateObjectsHashMap[estimateId].estimateData, parameters, path, updateAction);
    ...
    updateAnnotInHashMap(annot) {
        ...
        switch (type) {
            case ANNOT_TYPES.GROUP:
                ...
                break;
            case ANNOT_TYPES.REDUCTION:
                this.estimateObjectsHashMap[geoEstimateId].objects.reductionsMap[geoFile.id][parentId][id] = annot;
                break;
            default:
                this.estimateObjectsHashMap[geoEstimateId].objects.annotationsMap[geoFile.id][id] = annot;
                break;
        }
    },
    ...
    onRowsDelete(rows) {
        _.forEach(rows, (row) => {
            const currentRow = row.parentId
                ? this.estimateObjectsHashMap[row.estimateId]?.objects.reductionsMap[row.fileId][row.parentId][row.geoAnnotationId].rows[row.id]
                : this.estimateObjectsHashMap[row.estimateId]?.objects.annotationsMap[row.fileId][row.geoAnnotationId].rows[row.id];
    ...
    parseAnnotation(annot, estimate) {
        ...
        if (annot.type === ANNOT_TYPES.GROUP && !annot.parentId) annot.parentId = undefined;
            ...
            if (annot.type === ANNOT_TYPES.POINT) {
                ...
                annot.textContent = textContent;
                annot.fontSize = fontSize;
                annot.textFont = textFont;
                annot.TextColor = TextColor;
                annot.fontDecoration = fontDecoration;
            } else if (
    ...
    parseScale(scale, estimate, parameters, path, updateAction) {
        const isEstimateLocked = estimate?.locked;
        ...
        scale.readOnly = isEstimateLocked || readOnly;
        scale.Hidden = Hidden;
    ...
    parseAnnotationAttribute(xfdfString, key, obj = null) {
        ...
        switch (key) {
            ...
            case "setInitialValues":
                ...
                annotationElement.setAttribute("style", obj.style);
                annotationElement.setAttribute("width", obj.width);
                annotationElement.setAttribute("textFont", obj.textFont);
            ...
            case "getInitialValues": {
                ...
                let color = annotationElement.getAttribute("color");
                ...
                if (obj.type === ANNOT_TYPES.POINT) {
                    ...
                    _.forEach(textAnnot, (annotElement) => {
                        textContent = annotElement.childNodes[0].nodeValue;
                    });
                    ...
                    TextColor = annotationElement.getAttribute("TextColor");
                    fontDecoration = AnnotationStore.parseRichTextStyleFromXfdf(xfdfString);
                } else if (
    ...
    parseRows(rows, annotation) {
        ...
        for (const [key, row] of Object.entries(rows)) {
            ...
            if (!rowData.unit) rowData.unit = CalculationStore.setUnitForRow(rowData) || "";
            ...
            rows[key].annotationName = annotation.name;
            rows[key].annotationNumber = annotation.number;
    ...
    calculateValues(annotation, scales, reductions) {
        ...
        switch (annotation.type) {
            ...
            case ANNOT_TYPES.POLYGON:
                ...
                const reductionsList = _.get(reductions, [annotation.geoFile.id, annotation.annotationId]);
                if (reductionsList)
                    _.values(reductionsList)
            ...
            case ANNOT_TYPES.FREE_TEXT: {
                ...
                annotation.annotationData = {};
                annotation.annotationData["ESTIMATE.ANNOTATION_VALUES.TEXT_CONTENTS"] = annotation.textContent;
                annotation.annotationData["ESTIMATE.ANNOTATION_PROPERTIES.FILE_NAME"] = fileName;
                return annotation;
            }
            default:
        ...
        _.forEach(selectedAnnots, (annot) => {
            if (annot.type !== ANNOT_TYPES.GROUP && _.isEmpty(sumCalculateValues)) {
                ...
                if (annot.type !== ANNOT_TYPES.X_SCALE && annot.type !== ANNOT_TYPES.Y_SCALE) {
                    ...
                    sumCalculateArray = [];
                    sumCalculateArray.push(sumCalculateValues);
                    sumCalculateValues = this.minfiedSummation(sumCalculateArray);
                }
            }
        });
