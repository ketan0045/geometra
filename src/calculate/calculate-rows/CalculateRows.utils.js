...
export const transformExtendedValue = (value, rowData) => {
    let annot;
    if (rowData.parentId)
        annot = ObjectsStore.getReductionByPDFTronAnnot({
            geoEstimateId: rowData.estimateId,
            geoFileId: rowData.fileId,
            geoAnnotId: rowData.id,
            geoParentId: rowData.parentId,
        });
    else annot = ObjectsStore.getAnnotationByPDFTronAnnot({ geoEstimateId: rowData.estimateId, geoFileId: rowData.fileId, geoAnnotId: rowData.id });

    const annotData = annot.annotationData;

    return CalculationStore.parseExpressionToValues(value, Immutable.fromJS(annotData));
};
export const generateRowLibraryBody = (data) => {
    ...
    return [
        {
            ...
            filterDropdown: (event) => (
                <FilterDropdown
                    object="status"
                    annotationRows={generateRowLibraryBody(annotationRows)}
                    event={event}
        ...
        },
        {
            ...
            filterDropdown: (event) => (
                <FilterDropdown object="material" annotationRows={generateRowLibraryBody(annotationRows)} event={event} onChangeFilters={onChangeFilterState} />
            ),
            filterIcon: () => <FontAwesomeIcon icon={["fal", "filter"]} />,
        ...
        },
        {
            ...
            filterDropdown: (event) => (
                <FilterDropdown object="unit" annotationRows={generateRowLibraryBody(annotationRows)} event={event} onChangeFilters={onChangeFilterState} />
            ),
            filterIcon: () => <FontAwesomeIcon icon={["fal", "filter"]} />,
