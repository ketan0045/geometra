...
class CalculateProperties extends React.PureComponent {
    ...
    saveAngles = (values) => {
        if (this.validateAngles()) {
            const annotationsToUpdate = getSelectedAnnotations(this.state.selectedAnnotations, false, false);
            const parsedAngle = this.translateImperial(values);
            const angleFormula = "/cos(" + parsedAngle + ")";
            let formulaVariables = [];

            if (this.state.areaChecked) formulaVariables.push("NA");
            if (this.state.volumeChecked) formulaVariables.push("NVO");
            if (this.state.lengthChecked) formulaVariables.push("NL");
            if (this.state.wallChecked) formulaVariables.push("NV");

            if (annotationsToUpdate.length > 0) AnnotationStore.changeFormula(annotationsToUpdate, angleFormula, formulaVariables);
