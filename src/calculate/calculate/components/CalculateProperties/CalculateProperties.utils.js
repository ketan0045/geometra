...
import { DECORATION_LIST_COMPARE } from "constants/FontConstants";
...
export const isScaleSelected = () => {
    ...
    return typeMap[ANNOT_TYPES.X_SCALE] || typeMap[ANNOT_TYPES.Y_SCALE];
};
...
export const getAttributesValue = (selectedItems, key) => {
    ...
    switch (key) {
        ...
        case ANNOT_ATTRIBUTES.FONT_DECORATION: {
            const firstValue = Object.values(getValue(selectedItems[0], key));
            const IsEveryHaveSameValue = _.every(selectedItems, (item) => getValue(item, key) === firstValue);
            return IsEveryHaveSameValue ? firstValue : _.filter(firstValue, (value) => DECORATION_LIST_COMPARE.includes(value));
        }
