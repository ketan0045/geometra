...
export default class GeometricCalculation {
    constructor(height, quantity, xScale, geoFile, yScale = undefined) {
        ...
        this.yScale = 1;
        if (xScale) {
            this.xScale = this.getScaleValueFromXfdf(xScale.xdf, xScale.length);
            this.yScale = this.xScale;
        }
        if (yScale) this.yScale = this.getScaleValueFromXfdf(yScale.xdf, yScale.length);

        this.quantity = quantity;
        this.fileName = undefined;
        if (geoFile) {
            const fileId = typeof geoFile === "number" ? geoFile : geoFile.id;
    ...
    getVerticesFromXfdf(xfdfString) {
        ...
        if (!element) element = xfdfElements.getElementsByTagName("gesture")[0];
    ...
    getCenterPoint(annotation) {
        const rect = this.getRectFromXfdf(annotation.xfdf || annotation.xdf);
        const x = rect[0];
        const y = rect[1];
        const x1 = rect[2];
        const y1 = rect[3];
    ...
    calculatePolygonArea(vertices) {
        ...
        if (this.quantity < 0) return -1 * area;
    ...
    calculatePolygonPoints(vertices) {
        if (vertices && this.quantity != undefined) return (vertices.length - 1) * this.quantity;
    ...
    calculateFormulae(annotation) {
        if (annotation && annotation.annotationData) {
            if (annotation.formulaNA) annotation = this.calculateValueFormula(annotation, "ESTIMATE.ANNOTATION_VALUES.NET_AREA", "formulaNA");
            if (annotation.formulaNL) annotation = this.calculateValueFormula(annotation, "ESTIMATE.ANNOTATION_VALUES.NET_LENGTH", "formulaNL");
            if (annotation.formulaNVO) annotation = this.calculateValueFormula(annotation, "ESTIMATE.ANNOTATION_VALUES.NET_VOLUME", "formulaNVO");
            if (annotation.formulaNV) annotation = this.calculateValueFormula(annotation, "ESTIMATE.ANNOTATION_VALUES.NET_WALL", "formulaNV");
        }
        return annotation;
    }

    calculateValueFormula(annotation, keyValue, keyFormula) {
        let annotationData = annotation.annotationData;
        const parseExpression =
            isNaN(annotation[keyFormula]) || annotation[keyFormula].includes("+") || annotation[keyFormula].includes("-")
                ? CalculationStore.parseExpressionToValues(annotationData[keyValue] + annotation[keyFormula], Immutable.fromJS(annotationData))
                : annotation[keyFormula];
        annotationData[keyValue] = parseExpression;
    ...
    calculateTilesForAnnotation(annotation, totalReductionLength = 0, reductionTiles = 0) {
        ...
        if (annotation["ESTIMATE.ANNOTATION_PROPERTIES.AREA_TILES_X"] && annotation.annotationData) {
            const tileX = annotation["ESTIMATE.ANNOTATION_PROPERTIES.AREA_TILES_X"];
            const tileY = annotation["ESTIMATE.ANNOTATION_PROPERTIES.AREA_TILES_Y"];
            if (tileX && tileX > 0 && tileY && tileY > 0) {
                const selectedArea = annotationData["ESTIMATE.ANNOTATION_VALUES.NET_AREA"];
                const o = annotationData["ESTIMATE.ANNOTATION_VALUES.LENGTH"];
                const jointX = annotation["ESTIMATE.ANNOTATION_PROPERTIES.AREA_JOINT_X"];
                const jointY = annotation["ESTIMATE.ANNOTATION_PROPERTIES.AREA_JOINT_Y"];
                const jointDepth = annotation["ESTIMATE.ANNOTATION_PROPERTIES.AREA_JOINT_DEPTH"];
                ...
                annotationData["ESTIMATE.ANNOTATION_VALUES.AREA_TILES"] = nrTiles;
                annotationData["ESTIMATE.ANNOTATION_VALUES.AREA_JOINT_LENGTH"] = jointLength;
                annotationData["ESTIMATE.ANNOTATION_VALUES.AREA_JOINT_VOLUME"] = jointVolume;
                annotation.annotationData = annotationData;
            }
        }

        if (annotation["ESTIMATE.ANNOTATION_PROPERTIES.WALL_TILES_X"] && annotation["annotationData"]) {
            const tileX = annotation["ESTIMATE.ANNOTATION_PROPERTIES.WALL_TILES_X"];
            const tileY = annotation["ESTIMATE.ANNOTATION_PROPERTIES.WALL_TILES_Y"];
            if (tileX && tileX > 0 && tileY && tileY > 0) {
                const selectedWall = annotationData["ESTIMATE.ANNOTATION_VALUES.NET_WALL"];
                const o = annotationData["ESTIMATE.ANNOTATION_VALUES.LENGTH"];
                const jointX = annotation["ESTIMATE.ANNOTATION_PROPERTIES.WALL_JOINT_X"];
                const jointY = annotation["ESTIMATE.ANNOTATION_PROPERTIES.WALL_JOINT_Y"];
                const jointDepth = annotation["ESTIMATE.ANNOTATION_PROPERTIES.WALL_JOINT_DEPTH"];
    ...
    getNextNumberString(inputString) {
        if (this.isNumber(inputString)) {
            ...
            try {
                if (this.isNumber(inputString.charAt(inputString.length - 1))) return this.adjustNumeric(inputString);
                else return this.adjustCharacters(inputString);
            } catch (error) {
    ...
    adjustCharacters(inputString) {
        ...
        for (let i = inputArray.length - 1; i >= 0; i--) {
            ...
            if (this.isLetter(charCode)) {
                if ((charCode > 64 && charCode < 90) || (charCode > 96 && charCode < 122)) {
                    ...
                    if (inputArray[inputArray.length - 1].charCodeAt(0) <= 90) inputArray[i] = "A";
                    else if (inputArray[inputArray.length - 1].charCodeAt(0) <= 122) inputArray[i] = "a";
                }
            } else {
                break;
            }
        ...
        if (!updated) {
            if (inputArray[inputArray.length - 1].charCodeAt(0) <= 90) inputArray.push("A");
            else if (inputArray[inputArray.length - 1].charCodeAt(0) <= 122) inputArray.push("a");
        }
    ...
    getNumberIndexFromLeft(inputArray) {
        for (let i = inputArray.length - 1; i >= 0; i--) {
            if (!this.isNumber(inputArray[i])) return i + 1;
        }
        return 0;
    }
}
