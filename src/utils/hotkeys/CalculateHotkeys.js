...
export const handleDoubleClick = () => {
    if (AnnotationStore.getCurrentToolNode() === "Pan") return;
    setTimeout(() => {
...
export const handleCalculateKeyDown = (event) => {
    ...
    switch (keyCode) {
        ...
        case 40: // ArrowDown
            ...
            const { selectionKeys } = ObjectsStore.getSelectionList();
            const index = _.indexOf(data.keys, selectionKeys[selectionKeys.length - 1]);
            ...
            if (data.keys[keyIdex]) {
                const newlySelectedNode = data.nodes[keyIdex];
                if (newlySelectedNode.data.type === ANNOT_TYPES.GROUP) {
                    ObjectsStore.selectListOfObjects([], [newlySelectedNode]);
                } else {
                    ObjectsStore.selectListOfObjects([newlySelectedNode], []);
                }
                TreeStoreV2.getTreeRef().current.scrollTo({ key: data.keys[keyIdex], align: "top", offset: 100 });
            }
            break;
        case 37: // ArrowLeft
            if (altKey && !isActiveIfcFile) {
