...
class Drive extends React.PureComponent {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (prevState?.driveLoader.isLoading !== this.state?.driveLoader?.isLoading) {
            ...
            const getDriveNode = FileStore.getFileById(parseInt(driveSelectedKey)) || FileStore.getRootFolder();
    ...
    fileStoreUpdated = (message, fileId, index) => {
        switch (message) {
            ...
            case "updateSelectedNode":
                ...
                break;
            case "createNewSelectedNode":
                let newFolderId = FileStore?.currentFolder?.id;
                if (newFolderId) {
                    let getSelectedNode = FileStore.getSelectedNode(newFolderId);
                    this.setState(
                        {
                    ...
                    FileStore.reloadFileView([getSelectedNode]);

                    // Expanded Parent node
                    let setExpandedKeys = [...this.state.expandedKeys, getSelectedNode.parentId];
                    this.onExpandDriveTree(setExpandedKeys);

                    const uniqueArray = [...new Set([getSelectedNode])];
                    this.setState({ checkedItems: uniqueArray });
                    localStorage.setItem(`driveSelected_${this.props.match.params.projectId}`, getSelectedNode.key);

                    FileActions.setSelectedFileNodes([getSelectedNode]);
                    FileStore.setCheckedItemsIds([getSelectedNode.key]);
                    AnnotationActions.setActiveFileId(getSelectedNode.key);
                }
                break;
            ...
            case "treeNodeDeleted":
                {
                    if (FileStore.getSelectedNode(this.state.checkedItems[0].parentId)) {
                        this.setState(
                            {
    ...
    onCollapseAll = () => {
        ...
        let expandedKeys = [];
        if (rootFolder.id) expandedKeys.push(rootFolder.id);
        this.setState({ expandedKeys: expandedKeys, allKeysExpanded: false });
        localStorage.setItem(`pdfExpandedKeys_${this.props.match.params.projectId}`, JSON.stringify(expandedKeys));
    };
    ...
    onExpandDriveTree = (expandedKeys) => {
        const projectId = parseInt(this.props.match.params.projectId, 10);
        let rootFolder = FileStore.getRootFolder();
        if (!expandedKeys.includes(rootFolder.id)) expandedKeys.push(rootFolder.id);
